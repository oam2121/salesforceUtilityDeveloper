...
def show_advanced_soql_query_builder(sf):
    ...
    if parent_object:
        try:
            ...
            # Parent order and limit
            st.subheader("Parent Advanced Filters")
            parent_order_by = st.selectbox("Parent Order By", selected_parent_fields)
            parent_order_direction = st.selectbox("Order Direction", ["ASC", "DESC"])
            parent_limit = st.number_input("Parent Limit", min_value=1, value=100)
    ...
    if st.button("Run Query"):
        ...
        try:
            ...
            if summary_mode:
                ...
                for record in records:
                    st.markdown(f"### Account: {record.get('Name', 'No Name')}")
                    account_data = {field: record.get(field) for field in selected_parent_fields}
